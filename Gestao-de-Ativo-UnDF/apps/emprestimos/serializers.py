...
class EmprestimoCreateSerializer(serializers.ModelSerializer):
    ...
    def create(self, validated_data):
        ...
        if not request or not request.user.get_servidor_profile():
...
class SolicitacaoEmprestimoCreateSerializer(serializers.ModelSerializer):
    class Meta:
        model = Emprestimo
        fields = ['ativo', 'data_devolucao_prevista', 'observacao_saida']

    def create(self, validated_data):
        request = self.context.get('request')
        if not request or not request.user or not request.user.is_authenticated:
            raise BusinessValidationError("Usuário não autenticado.")
            
        return EmprestimoService.solicitar_emprestimo(
            ativo=validated_data['ativo'],
...
class EmprestimoRejectionSerializer(serializers.Serializer):
    motivo = serializers.CharField(max_length=500, required=False, allow_blank=True, allow_null=True)
